...
- different weapon allocation policies (by default, everyone gets an AK/M4, armor, a helmet, and a kit)
- change how players get put into teams by changing their "round points" or the team ratios
...
- use ``sm_edit`` to launch into edit mode, this makes more spawn-editing commands avaliable (you shouldn't do this on a live, public server)
...
- use ``sm_bomb`` to toggle whether to display all spawns or potential bomb-carrier spawns
- use ``sm_nobomb`` to mark a spawn as unavailable to a bomb carrier. (by default, all bombs within the x-y plane of a bombsite are available for a bomb carrier)
